		...
		try {
			...
			for (final Class clazz : Play.classloader.getAllClasses()) {
				...
				try {
					...
					if (clazz.getName().contains("Plugin") || clazz.getName().contains("config.Guice")) {
						continue;
					}

					Logger.debug(String.format("%s will load in load guice", clazz.getName()));
					...
					Logger.error(String.format("%s is error in load guice", clazz.getName()));
					throw e;
				}
			}
